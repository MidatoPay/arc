import { createContext, useContext, useMemo, useState } from "react";

const STORAGE_KEY = "mp_lang";

export function localeFor(lang) {
  return lang === "en" ? "en-US" : "es-AR";
}

export const STACK_EN = [
  { on: true, name: "Arc L1 (testnet)", role: "This demo's settlement chain. Chain ID 5042002, sub-second finality.", by: "Circle" },
  { on: true, name: "Native USDC", role: "Gas and payment asset in one: every transfer in this app is native USDC on-chain.", by: "Circle" },
  { on: true, name: "On-chain reference", role: "Every charge carries its invoice number attached. Reconciliation without an external database.", by: "Arc" },
  { on: true, name: "Privy", role: "This app's login: email or phone → embedded wallet. No recovery phrases.", by: "Arc ecosystem" },
  { on: true, name: "ArcScan", role: "Every payment is verifiable at testnet.arcscan.app, with its reference included.", by: "Arc ecosystem" },
  { on: true, name: "Circle faucet", role: "Test USDC funding from faucet.circle.com.", by: "Circle" },
  { on: false, name: "Memo contract", role: "Migrate the reference to indexed Memo events for analytics and bulk reconciliation.", by: "Arc" },
  { on: false, name: "Gateway / CCTP v2", role: "Cross-chain USDC: on-ramp from Solana or Base, settlement on Arc.", by: "Circle" },
  { on: true, name: "FX oracle", role: "ARS/USD from Chainlink Price Feed on Ethereum Mainnet (latestAnswer).", by: "Chainlink" },
  { on: false, name: "StableFX / EURC", role: "Multi-currency corridor with Arc's FX engine for euro-denominated charges.", by: "Circle" },
  { on: false, name: "Aave", role: "Yield on the USDC treasury float.", by: "Arc ecosystem" },
  { on: false, name: "Avenia", role: "LATAM ramps (PIX and local rails) for expansion into Brazil.", by: "Arc ecosystem" },
];

export const STACK_ES = [
  { on: true, name: "Arc L1 (testnet)", role: "Cadena de liquidación de esta demo. Chain ID 5042002, finalidad sub-segundo.", by: "Circle" },
  { on: true, name: "USDC nativo", role: "Gas y activo de pago en uno: cada envío de esta app es USDC nativo on-chain.", by: "Circle" },
  { on: true, name: "Referencia on-chain", role: "Cada cobro lleva su número de factura adjunto. Conciliación sin base de datos externa.", by: "Arc" },
  { on: true, name: "Privy", role: "El login de esta app: email o teléfono → wallet embebida. Sin frases de recuperación.", by: "Ecosistema Arc" },
  { on: true, name: "ArcScan", role: "Cada pago es verificable en testnet.arcscan.app, con su referencia incluida.", by: "Ecosistema Arc" },
  { on: true, name: "Faucet Circle", role: "Fondeo de USDC testnet desde faucet.circle.com.", by: "Circle" },
  { on: false, name: "Memo contract", role: "Migrar la referencia a eventos Memo indexados para analítica y conciliación masiva.", by: "Arc" },
  { on: false, name: "Gateway / CCTP v2", role: "USDC crosschain: entrada desde Solana o Base, liquidación en Arc.", by: "Circle" },
  { on: true, name: "Oráculo FX", role: "ARS/USD desde el Price Feed de Chainlink en Ethereum Mainnet (latestAnswer).", by: "Chainlink" },
  { on: false, name: "StableFX / EURC", role: "Corredor multi-moneda con el motor FX de Arc para cobros en euros.", by: "Ecosistema Arc" },
  { on: false, name: "Aave", role: "Rendimiento sobre el float de tesorería en USDC.", by: "Ecosistema Arc" },
  { on: false, name: "Avenia", role: "Rampas LATAM (PIX y rieles locales) para expansión a Brasil.", by: "Ecosistema Arc" },
];

const translations = {
  en: {
    login: {
      welcome: "Welcome to",
      subtitle: "The new way to get paid.",
      loginBtn: "Sign in",
      loadingBtn: "Loading…",
      createAccountBtn: "Create account",
    },
    onboarding: {
      getStarted: "Get started",
      skipToLogin: "I already have an account",
      welcomeEyebrow: "If you have a business:",
      welcomeTitle: "The future of\npayments is here",
      welcomeSub: "",
      progressAria: "Onboarding progress",
      slideAria: (n) => `Slide ${n}`,
      prev: "Previous",
      next: "Next",
      slides: [
        {
          eyebrow: "The pain",
          before: "Getting paid and going dollar shouldn’t cost you ",
          highlight: "time",
          after: ".",
        },
        {
          eyebrow: "How they pay",
          before: "QR, CBU or alias. Your customer pays in ",
          highlight: "ARS",
          after: ".",
        },
        {
          eyebrow: "The gain",
          before: "You collect in pesos. You settle in ",
          highlight: "dollars",
          after: ".",
        },
        {
          eyebrow: "Control",
          before: "Your money. Your ",
          highlight: "wallet",
          after: ". No black box.",
        },
        {
          eyebrow: "Ready",
          before: "The future of getting paid, in your ",
          highlight: "commerce",
          after: ".",
        },
      ],
    },
    home: {
      greeting: (nombre) => `Hi, ${nombre}`,
      greetingHi: "Hi,",
      available: "Available",
      availableBreakdown: (ars, usdc) => `ARS ledger $${ars} · USDC ${usdc}`,
      fxLine: (rate) => `1 USDC = $${rate} ARS · Chainlink`,
      arsSimulated: "Simulated ARS balance",
      treasuryTitle: "Treasury",
      actionReceive: "Collect",
      actionConvert: "Convert",
      actionPay: "Pay",
      actionSend: "Send",
      fundsTitle: "Funds",
      viewAll: "View all",
      infoDigitalAssets: "Your money is held in digital assets.",
      copy: "copy",
      copied: "copied ✓",
      faucetCardTitle: "Load test funds to get started",
      faucetCardBody: "Copy your address and request USDC from the Circle faucet, selecting Arc Testnet.",
      faucetCardBtn: "Go to faucet",
      activityTitle: "Activity",
      activityEmpty: "No activity yet. Try collect, convert, or a voice payment — every transfer is recorded on Arc Testnet.",
      invoiceLink: (factura) => `Invoice ${factura} · view ↗`,
      creatingTitle: "Creating your account…",
      creatingBody: "This takes a few seconds the first time.",
    },
    charge: {
      title: "Collect",
      subtitle: "Enter an amount and show the QR — whoever scans it pays you directly, wallet to wallet.",
      arsLabel: "Amount in ARS",
      youReceive: "You receive (approx.)",
      cta: "Generate QR",
      waitingTitle: "Waiting for payment…",
      waitingHint: "Keep this screen open. It updates automatically once the payment arrives.",
      linkLabel: "Or share this link",
      copyLink: "Copy link",
      linkCopied: "Link copied ✓",
      cancel: "Cancel",
      merchantSelf: "Your commerce",
      detectError: "Payment detected on-chain, but the exact transaction couldn't be located. Check ArcScan for your address.",
    },
    pay: {
      title: "Pay",
      recipientLabel: "Alias or address",
      recipientPlaceholder: "katy or 0x…",
      recipientInvalid: "Enter a contact alias or a valid wallet address.",
      amountUsdc: "Amount in USDC",
      amountArs: "Amount in ARS",
      amountTooLow: "Minimum is 0.01 USDC.",
      youSend: "You send",
      cta: "Pay",
      working: "Sending…",
      modeManual: "Manual",
      modeScan: "Scan QR",
      scanHint: "Point the camera at the QR code",
      scanCameraError: "Camera unavailable. Paste the link instead.",
      scanLinkPlaceholder: "Paste the payment link",
      scanLinkCta: "Use link",
      invalidQr: "Invalid QR code.",
      selfPay: "You can't pay your own QR code.",
      payTo: (who) => `Pay ${who}`,
      confirmPayCta: "Confirm and pay",
    },
    send: {
      title: "Send",
      cta: "Send",
      viewFeeDetails: "View details",
      hideFeeDetails: "Hide details",
    },
    convert: {
      title: "Convert",
      subtitle: "Move between ARS and USDC using the live Chainlink rate.",
      arsToUsdc: "ARS → USDC",
      usdcToArs: "USDC → ARS",
      sell: "Sell",
      buy: "Buy",
      max: "Max",
      flip: "Flip direction",
      amountArs: "Amount in ARS",
      amountUsdc: "Amount in USDC",
      youGet: "You get",
      yourArs: "Your ARS",
      yourUsdc: "Your USDC",
      hintArsUsdc: "The treasury sends USDC to your wallet. Simulated ARS is debited when available.",
      hintUsdcArs: "You send USDC to the treasury. ARS is credited locally (no real bank transfer).",
      cta: "Convert",
      ctaSign: "Confirm and sign",
      working: "Converting…",
      workingHint: "Processing your conversion…",
      treasuryLabel: "Conversion",
      treasuryMissing: "Treasury wallet not configured. Set VITE_TREASURY_PRIVATE_KEY in .env (testnet only).",
      feeTitle: "Network fee",
      feeDeducted: "Taken from your USDC amount",
      feePending: "Estimating fee…",
    },
    gas: {
      title: "Estimated network fee",
      estimating: "Estimating gas…",
      gasLimit: "Gas limit",
      gasPrice: "Gas price",
      maxFee: "Max fee per gas",
      maxPriority: "Priority fee",
      totalNative: "Est. total (native)",
      totalUsd: "Est. total (USD)",
      totalArs: "Est. total (ARS)",
      retry: "Retry estimate",
      walletMissing: "Wallet not connected — can't estimate gas.",
      disclaimer: "Estimate from the network. The final fee may vary slightly at confirmation.",
    },
    voice: {
      title: "Pay by voice",
      subtitle: "Say it and the agent builds the transaction.",
      idleHint: "Tap the mic and say the payment",
      listening: "Listening…",
      listeningPlaceholder: "Say the payment…",
      listeningHint: "When you're done, wait a second",
      fabListeningHint: "Tap the mic again to stop",
      parsing: (transcript) => `Interpreting "${transcript}"…`,
      orType: "Or type it",
      inputPlaceholder: "send 1 dollar to katy",
      exampleCommand: "send 1 dollar to katy",
      tryExample: "▶ Try “send 1 dollar to katy”",
      noApiKeyHint: "No API key: using local parser.",
      micUnsupported: "Your browser doesn't support voice recognition. Use Chrome or text mode.",
      noSpeechMatch: "Couldn't understand the audio. Speak closer to the microphone.",
      micErrors: {
        "not-allowed": "Microphone permission denied. In Safari: Settings → Websites → Microphone → localhost: Allow. And enable Dictation in System Settings → Keyboard.",
        "service-not-allowed": "Safari needs Dictation enabled: System Settings → Keyboard → Dictation.",
        "audio-capture": "No microphone detected.",
        "no-speech": "I didn't hear anything. Tap the button and speak right away.",
        network: "Check your internet connection.",
      },
      micErrGeneric: (e) => `Microphone error: ${e}`,
      micUnavailable: "Voice recognition isn't available. Use text mode.",
      noPaymentUnderstood: "I didn't understand a payment in that command. Try: “send 1 dollar to katy” or “charge 500 pesos”.",
      aliasNotFound: (alias) => `I couldn't find the alias “${alias}” in your contacts.`,
      noContactsYet: "You don't have any contacts saved yet. Go to Contacts and add one with a wallet address and alias.",
      amountTooLow: (amount, currency) => `That came out to less than 0.01 USDC (heard "${amount} ${currency}"). It probably misheard the currency — try again saying "dollars" or "USDC" clearly.`,
      insufficientBalance: (usdc, balance) => `Insufficient balance: you're trying to send ${usdc} USDC and you have ${balance}.`,
      amount: "Amount",
      equals: "Equals",
      exchangeRate: "Exchange rate",
      invoice: "Invoice",
      network: "Network",
      memoNote: "The invoice travels attached to the payment and is visible on ArcScan. The merchant reconciles without an external database.",
      confirmSend: "Confirm and send",
      cancel: "Cancel",
      sending: "Sending…",
      sendingHint: "Signing and waiting for finality on Arc",
      retry: "Retry",
      rpcLimited: "Arc's RPC is rate-limiting requests. Wait a few seconds and try again.",
      insufficientFundsOnchain: "Insufficient funds on-chain. Fund your wallet at faucet.circle.com → Arc Testnet.",
      signatureCancelled: "You canceled the signature.",
      txFailed: (m) => `The transaction failed: ${m}`,
    },
    success: {
      splashTitle: "Payment sent!",
      splashCharge: "Payment collected!",
      splashConvert: "Conversion done!",
      doneTitle: "Done!",
      sentSummary: (usdc, name) => `You sent ${usdc} USDC to ${name}`,
      chargeSummary: (ars, usdc) => `Collected $${ars} ARS → received ${usdc} USDC`,
      convertArsUsdcSummary: (ars, usdc) => `Converted $${ars} ARS → ${usdc} USDC`,
      convertUsdcArsSummary: (usdc, ars) => `Converted ${usdc} USDC → $${ars} ARS`,
      arsCredited: (ars) => `ARS credited: $${ars}`,
      viewDetail: "View detail",
      operation: "Transaction:",
      amountSent: "Amount sent",
      equals: "Equals",
      exchangeRate: "Exchange rate",
      networkFee: "Network fee",
      block: "Block",
      time: "Time",
      invoiceLabel: "Invoice:",
      onchainCheck: "on-chain ✓",
      viewOnArcScan: (short) => `${short} — view on ArcScan ↗`,
      utf8Hint: "On ArcScan, open Input Data and switch it to UTF-8: that's where the full payment reference is.",
      anotherPayment: "Make another payment",
      anotherCharge: "Collect again",
      anotherConvert: "Convert again",
      backHome: "Back to home",
    },
    movs: {
      title: "Activity",
      countEmpty: "No payments yet in this session.",
      countN: (n) => `${n} payment${n === 1 ? "" : "s"} this session`,
      emptyBody: "Collect, convert, and voice payments are recorded on Arc Testnet with an invoice number.",
      voicePayment: "Voice payment",
      charge: "Collected",
      convert: "Conversion",
      convertArsUsdc: "Convert ARS → USDC",
      convertUsdcArs: "Convert USDC → ARS",
      invoiceLabel: "Invoice:",
      equals: "Equals:",
      viewOnArcScan: (short) => `${short} — view on ArcScan ↗`,
      viewFullHistory: "View full history on ArcScan",
    },
    mas: {
      title: "More",
      noEmailAccount: "Account with no email",
      stackTitle: "Arc Stack",
      stackSub: "What's built and what's missing",
      faucetTitle: "Top up with the faucet",
      faucetSub: "Test USDC on Arc Testnet",
      arcscanTitle: "View my account on ArcScan",
      arcscanSub: "Arc's public explorer",
      languageTitle: "Language",
      logoutTitle: "Log out",
      footerLine1: "MidatoPay × Arc · demo on Arc Testnet",
      footerLine2: "Test funds with no real value",
      logoutConfirmTitle: "Log out?",
      logoutConfirmBody: "You'll go back to the home screen. You can sign in again with the same email or phone and your account stays the same.",
      logoutConfirmBtn: "Log out",
      cancel: "Cancel",
    },
    stackScreen: {
      title: "Arc Stack",
      subtitle: "Green is implemented and working in this demo. Gray isn't yet — we list it to be explicit about what's missing.",
      implemented: "IMPLEMENTED",
      roadmap: "ROADMAP",
    },
    agenda: {
      title: "Contacts",
      subtitle: "Save wallet addresses so voice payments can find them by alias.",
      searchPlaceholder: "Search by name or alias",
      addButton: "+ Add contact",
      emptyTitle: "No contacts yet",
      emptyBody: "Add a contact with a wallet address and alias to start paying by voice.",
      copy: "copy",
      copied: "copied ✓",
      form: {
        nameLabel: "Name",
        namePlaceholder: "Katy R.",
        aliasLabel: "Alias (used in voice commands)",
        aliasPlaceholder: "katy",
        addressLabel: "Wallet address",
        addressPlaceholder: "0x…",
        noteLabel: "Note (optional)",
        notePlaceholder: "Deenex — CFO",
        save: "Save contact",
        saveNew: "Add contact",
        cancel: "Cancel",
        delete: "Delete contact",
      },
      errors: {
        name: "Enter a name.",
        alias: "Enter an alias.",
        aliasDuplicate: "You already have a contact with that alias.",
        addr: "That doesn't look like a valid wallet address.",
        server: "Couldn't save — check your connection and try again.",
      },
    },
    qr: {
      saveContactTitle: "Save as contact?",
      saveContactBody: (who) => `Save ${who} so you can pay them faster next time.`,
      saveContactYes: "Save contact",
      saveContactNo: "Not now",
    },
    guest: {
      name: "Customer",
      scanCta: "Scan QR",
      scanTitle: "Pay with QR",
      useLink: "Use link",
      useLinkTitle: "Paste payment link",
      useLinkBody: "Paste the charge link to continue.",
      useLinkCta: "Continue",
      flash: "Toggle flashlight",
      backHome: "Back",
      exit: "Exit",
      treasuryMissing: "Treasury wallet not configured.",
      insufficient: "Not enough ARS available in the treasury for this payment.",
      paying: "Paying…",
      confirmCta: "Confirm payment",
      youPay: "You pay",
      merchant: "Merchant",
      payToLabel: "Pay to",
      merchantGets: "Merchant receives",
      paidSummary: (who, ars) => `You paid ${who} $${ars} ARS`,
      successSplash: "Payment successful!",
      successTitle: "Payment successful",
      invalidTitle: "Invalid QR",
      invalidBody: "Make sure it’s the right one and scan again.",
      scanAgain: "Scan again",
      goHome: "Go home",
    },
    nav: {
      home: "Home",
      movements: "Activity",
      stack: "Stack",
      agenda: "Contacts",
      more: "More",
      profile: "Profile",
      voiceAria: "Pay by voice",
    },
    install: {
      title: "Install MidatoPay",
      body: "Add the app to your home screen for faster access.",
      cta: "Install",
      dismiss: "Not now",
      iosTitle: "Install MidatoPay",
      iosBody: "On iPhone or iPad, tap the Share button and choose \"Add to Home Screen\".",
      iosStep1: "Tap Share",
      iosStep2: "Choose \"Add to Home Screen\"",
      iosCta: "Got it",
    },
  },
  es: {
    login: {
      welcome: "Bienvenido a",
      subtitle: "La nueva forma de cobrar.",
      loginBtn: "Iniciar sesión",
      loadingBtn: "Cargando…",
      createAccountBtn: "Crear cuenta",
    },
    onboarding: {
      getStarted: "Empezar",
      skipToLogin: "Ya tengo cuenta",
      welcomeEyebrow: "Si tenés un comercio:",
      welcomeTitle: "El futuro de los\npagos llegó",
      welcomeSub: "",
      progressAria: "Progreso del onboarding",
      slideAria: (n) => `Slide ${n}`,
      prev: "Anterior",
      next: "Siguiente",
      slides: [
        {
          eyebrow: "El dolor",
          before: "Cobrar y dolarizarte no debería costarte ",
          highlight: "tiempo",
          after: ".",
        },
        {
          eyebrow: "Cómo pagan",
          before: "QR, CBU o alias. Tu cliente paga en ",
          highlight: "ARS",
          after: ".",
        },
        {
          eyebrow: "El beneficio",
          before: "Vos cobrás en pesos. Liquidás en ",
          highlight: "dólares",
          after: ".",
        },
        {
          eyebrow: "Control",
          before: "Tu plata. Tu ",
          highlight: "wallet",
          after: ". Sin caja negra.",
        },
        {
          eyebrow: "Listo",
          before: "El futuro de los cobros, en tu ",
          highlight: "comercio",
          after: ".",
        },
      ],
    },
    home: {
      greeting: (nombre) => `Hola, ${nombre}`,
      greetingHi: "Hola,",
      available: "Disponible",
      availableBreakdown: (ars, usdc) => `ARS ledger $${ars} · USDC ${usdc}`,
      fxLine: (rate) => `1 USDC = $${rate} ARS · Chainlink`,
      arsSimulated: "Saldo ARS simulado",
      treasuryTitle: "Recaudadora",
      actionReceive: "Cobrar",
      actionConvert: "Convertir",
      actionPay: "Pagar",
      actionSend: "Enviar",
      fundsTitle: "Fondos",
      viewAll: "Ver todo",
      infoDigitalAssets: "Tu dinero se encuentra en activos digitales.",
      copy: "copiar",
      copied: "copiada ✓",
      faucetCardTitle: "Cargá saldo de prueba para empezar",
      faucetCardBody: "Copiá tu dirección y pedí USDC en el faucet de Circle eligiendo Arc Testnet.",
      faucetCardBtn: "Ir al faucet",
      activityTitle: "Actividad",
      activityEmpty: "Todavía no hay movimientos. Probá cobrar, convertir o un pago por voz — cada envío queda en Arc Testnet.",
      invoiceLink: (factura) => `Factura ${factura} · ver ↗`,
      creatingTitle: "Creando tu cuenta…",
      creatingBody: "Esto tarda unos segundos la primera vez.",
    },
    charge: {
      title: "Cobrar",
      subtitle: "Ingresá un monto y mostrá el QR — quien lo escanee te paga directo, wallet a wallet.",
      arsLabel: "Monto en ARS",
      youReceive: "Recibís (aprox.)",
      cta: "Generar QR",
      waitingTitle: "Esperando el pago…",
      waitingHint: "Dejá esta pantalla abierta. Se actualiza sola apenas llega el pago.",
      linkLabel: "O compartí este link",
      copyLink: "Copiar link",
      linkCopied: "Link copiado ✓",
      cancel: "Cancelar",
      merchantSelf: "Tu comercio",
      detectError: "Se detectó el pago on-chain, pero no se pudo ubicar la transacción exacta. Revisá ArcScan para tu dirección.",
    },
    pay: {
      title: "Pagar",
      recipientLabel: "Alias o address",
      recipientPlaceholder: "katy o 0x…",
      recipientInvalid: "Ingresá un alias de contacto o una address válida.",
      amountUsdc: "Monto en USDC",
      amountArs: "Monto en ARS",
      amountTooLow: "El mínimo es 0.01 USDC.",
      youSend: "Enviás",
      cta: "Pagar",
      working: "Enviando…",
      modeManual: "Manual",
      modeScan: "Escanear QR",
      scanHint: "Apuntá la cámara al código QR",
      scanCameraError: "Cámara no disponible. Pegá el link en su lugar.",
      scanLinkPlaceholder: "Pegá el link de cobro",
      scanLinkCta: "Usar link",
      invalidQr: "Código QR inválido.",
      selfPay: "No podés pagar tu propio código QR.",
      payTo: (who) => `Pagarle a ${who}`,
      confirmPayCta: "Confirmar y pagar",
    },
    send: {
      title: "Enviar",
      cta: "Enviar",
      viewFeeDetails: "Ver detalles",
      hideFeeDetails: "Ocultar detalles",
    },
    convert: {
      title: "Convertir",
      subtitle: "Mové entre ARS y USDC con la cotización live de Chainlink.",
      arsToUsdc: "ARS → USDC",
      usdcToArs: "USDC → ARS",
      sell: "Vendés",
      buy: "Recibís",
      max: "Máx",
      flip: "Invertir dirección",
      amountArs: "Monto en ARS",
      amountUsdc: "Monto en USDC",
      youGet: "Recibís",
      yourArs: "Tu ARS",
      yourUsdc: "Tu USDC",
      hintArsUsdc: "La recaudadora envía USDC a tu wallet. Si tenés ARS simulado, se debita.",
      hintUsdcArs: "Enviás USDC a la recaudadora. Se acredita ARS en el ledger local (sin transferencia bancaria real).",
      cta: "Convertir",
      ctaSign: "Confirmar y firmar",
      working: "Convirtiendo…",
      workingHint: "Procesando tu conversión…",
      treasuryLabel: "Conversión",
      treasuryMissing: "Wallet recaudadora no configurada. Definí VITE_TREASURY_PRIVATE_KEY en .env (solo testnet).",
      feeTitle: "Fee de red",
      feeDeducted: "Descontado de tu monto en USDC",
      feePending: "Estimando fee…",
    },
    gas: {
      title: "Fee de red estimado",
      estimating: "Estimando gas…",
      gasLimit: "Gas limit",
      gasPrice: "Gas price",
      maxFee: "Max fee per gas",
      maxPriority: "Priority fee",
      totalNative: "Total est. (nativo)",
      totalUsd: "Total est. (USD)",
      totalArs: "Total est. (ARS)",
      retry: "Reintentar estimación",
      walletMissing: "Wallet no conectada — no se puede estimar el gas.",
      disclaimer: "Estimación de la red. El fee final puede variar levemente al confirmar.",
    },
    voice: {
      title: "Pagar por voz",
      subtitle: "Decilo y el agente arma la transacción.",
      idleHint: "Tocá el micrófono y decí el pago",
      listening: "Escuchando…",
      listeningPlaceholder: "Decí el pago…",
      listeningHint: "Al terminar, esperá un segundo",
      fabListeningHint: "Tocá el mic de abajo otra vez para cancelar",
      parsing: (transcript) => `Interpretando «${transcript}»…`,
      orType: "O escribilo",
      inputPlaceholder: "enviar 1 dólar a katy",
      exampleCommand: "enviar 1 dólar a katy",
      tryExample: "▶ Probar «enviar 1 dólar a katy»",
      noApiKeyHint: "Sin API key: usando intérprete local.",
      micUnsupported: "Tu navegador no soporta reconocimiento de voz. Usá Chrome o el modo texto.",
      noSpeechMatch: "No se entendió el audio. Hablá más cerca del micrófono.",
      micErrors: {
        "not-allowed": "Permiso de micrófono denegado. En Safari: Configuración → Sitios web → Micrófono → localhost: Permitir. Y activá Dictado en Configuración del Sistema → Teclado.",
        "service-not-allowed": "Safari necesita el Dictado activado: Configuración del Sistema → Teclado → Dictado.",
        "audio-capture": "No se detectó micrófono.",
        "no-speech": "No escuché nada. Tocá el botón y hablá enseguida.",
        network: "Verificá tu conexión a internet.",
      },
      micErrGeneric: (e) => `Error de micrófono: ${e}`,
      micUnavailable: "El reconocimiento de voz no está disponible. Usá el modo texto.",
      noPaymentUnderstood: "No entendí un pago en ese comando. Probá: «enviar 1 dólar a katy» o «cobrar 500 pesos».",
      aliasNotFound: (alias) => `No encontré el alias «${alias}» en tus contactos.`,
      noContactsYet: "Todavía no tenés contactos guardados. Andá a Agenda y cargá uno con su dirección de wallet y alias.",
      amountTooLow: (amount, currency) => `Eso dio menos de 0.01 USDC (entendí "${amount} ${currency}"). Seguramente confundió la moneda — probá de nuevo diciendo "dólares" o "USDC" bien claro.`,
      insufficientBalance: (usdc, balance) => `Saldo insuficiente: querés enviar ${usdc} USDC y tenés ${balance}.`,
      amount: "Monto",
      equals: "Equivale a",
      exchangeRate: "Tipo de cambio",
      invoice: "Factura",
      network: "Red",
      memoNote: "La factura viaja adjunta al pago y queda visible en ArcScan. El comercio concilia sin base de datos externa.",
      confirmSend: "Confirmar y enviar",
      cancel: "Cancelar",
      sending: "Enviando…",
      sendingHint: "Firmando y esperando finalidad en Arc",
      retry: "Reintentar",
      rpcLimited: "El RPC de Arc está limitando las consultas. Esperá unos segundos y reintentá.",
      insufficientFundsOnchain: "Fondos insuficientes on-chain. Fondeá en faucet.circle.com → Arc Testnet.",
      signatureCancelled: "Cancelaste la firma.",
      txFailed: (m) => `La transacción falló: ${m}`,
    },
    success: {
      splashTitle: "¡Pago enviado!",
      splashCharge: "¡Cobro recibido!",
      splashConvert: "¡Conversión lista!",
      doneTitle: "¡Hecho!",
      sentSummary: (usdc, name) => `Enviaste ${usdc} USDC a ${name}`,
      chargeSummary: (ars, usdc) => `Cobraste $${ars} ARS → recibiste ${usdc} USDC`,
      convertArsUsdcSummary: (ars, usdc) => `Convertiste $${ars} ARS → ${usdc} USDC`,
      convertUsdcArsSummary: (usdc, ars) => `Convertiste ${usdc} USDC → $${ars} ARS`,
      arsCredited: (ars) => `ARS acreditado: $${ars}`,
      viewDetail: "Ver detalle",
      operation: "Operación:",
      amountSent: "Monto enviado",
      equals: "Equivale a",
      exchangeRate: "Tipo de cambio",
      networkFee: "Fee de red",
      block: "Bloque",
      time: "Hora",
      invoiceLabel: "Factura:",
      onchainCheck: "on-chain ✓",
      viewOnArcScan: (short) => `${short} — ver en ArcScan ↗`,
      utf8Hint: "En ArcScan abrí Input Data y pasalo a UTF-8: ahí está la referencia completa del cobro.",
      anotherPayment: "Realizar otro pago",
      anotherCharge: "Cobrar de nuevo",
      anotherConvert: "Convertir de nuevo",
      backHome: "Volver al inicio",
    },
    movs: {
      title: "Movimientos",
      countEmpty: "Todavía no hay pagos en esta sesión.",
      countN: (n) => `${n} ${n === 1 ? "pago" : "pagos"} en esta sesión`,
      emptyBody: "Cobros, conversiones y pagos por voz quedan registrados en Arc Testnet con su factura.",
      voicePayment: "Pago por voz",
      charge: "Cobraste",
      convert: "Conversión",
      convertArsUsdc: "Conversión ARS → USDC",
      convertUsdcArs: "Conversión USDC → ARS",
      invoiceLabel: "Factura:",
      equals: "Equivale a:",
      viewOnArcScan: (short) => `${short} — ver en ArcScan ↗`,
      viewFullHistory: "Ver historial completo en ArcScan",
    },
    mas: {
      title: "Más",
      noEmailAccount: "Cuenta sin email",
      stackTitle: "Stack Arc",
      stackSub: "Qué está implementado y qué falta",
      faucetTitle: "Fondear con el faucet",
      faucetSub: "USDC de prueba en Arc Testnet",
      arcscanTitle: "Ver mi cuenta en ArcScan",
      arcscanSub: "Explorador público de Arc",
      languageTitle: "Idioma",
      logoutTitle: "Cerrar sesión",
      footerLine1: "MidatoPay × Arc · demo sobre Arc Testnet",
      footerLine2: "Fondos de prueba sin valor real",
      logoutConfirmTitle: "¿Cerrar sesión?",
      logoutConfirmBody: "Vas a volver a la pantalla de inicio. Podés entrar de nuevo con el mismo email o teléfono y tu cuenta sigue igual.",
      logoutConfirmBtn: "Cerrar sesión",
      cancel: "Cancelar",
    },
    stackScreen: {
      title: "Stack Arc",
      subtitle: "Lo verde está implementado y funcionando en esta demo. Lo gris todavía no: lo listamos para ser explícitos sobre qué falta.",
      implemented: "IMPLEMENTADO",
      roadmap: "ROADMAP",
    },
    agenda: {
      title: "Agenda",
      subtitle: "Guardá direcciones de wallet para que los pagos por voz las encuentren por alias.",
      searchPlaceholder: "Buscar por nombre o alias",
      addButton: "+ Agregar contacto",
      emptyTitle: "Todavía no tenés contactos",
      emptyBody: "Agregá un contacto con su dirección de wallet y un alias para empezar a pagar por voz.",
      copy: "copiar",
      copied: "copiada ✓",
      form: {
        nameLabel: "Nombre",
        namePlaceholder: "Katy R.",
        aliasLabel: "Alias (se usa en los comandos de voz)",
        aliasPlaceholder: "katy",
        addressLabel: "Dirección de wallet",
        addressPlaceholder: "0x…",
        noteLabel: "Nota (opcional)",
        notePlaceholder: "Deenex — CFO",
        save: "Guardar cambios",
        saveNew: "Agregar contacto",
        cancel: "Cancelar",
        delete: "Borrar contacto",
      },
      errors: {
        name: "Ingresá un nombre.",
        alias: "Ingresá un alias.",
        aliasDuplicate: "Ya tenés un contacto con ese alias.",
        addr: "Esa dirección de wallet no parece válida.",
        server: "No se pudo guardar — revisá tu conexión e intentá de nuevo.",
      },
    },
    qr: {
      saveContactTitle: "¿Guardar como contacto?",
      saveContactBody: (who) => `Guardá a ${who} para pagarle más rápido la próxima vez.`,
      saveContactYes: "Guardar contacto",
      saveContactNo: "Ahora no",
    },
    guest: {
      name: "Cliente",
      scanCta: "Escanear QR",
      scanTitle: "Pagar con QR",
      useLink: "Usar link",
      useLinkTitle: "Pegá el link de cobro",
      useLinkBody: "Pegá el link de cobro para continuar.",
      useLinkCta: "Continuar",
      flash: "Activar flash",
      backHome: "Volver",
      exit: "Salir",
      treasuryMissing: "Wallet recaudadora no configurada.",
      insufficient: "No hay ARS suficientes en la recaudadora para este pago.",
      paying: "Pagando…",
      confirmCta: "Confirmar pago",
      youPay: "Pagás",
      merchant: "Comercio",
      payToLabel: "Pagás a",
      merchantGets: "El comercio recibe",
      paidSummary: (who, ars) => `Pagaste a ${who} $${ars} ARS`,
      successSplash: "¡Pago exitoso!",
      successTitle: "Pago exitoso",
      invalidTitle: "El QR es inválido",
      invalidBody: "Fíjate que sea el correcto y volvé a escanear.",
      scanAgain: "Volver a escanear",
      goHome: "Ir a inicio",
    },
    nav: {
      home: "Inicio",
      movements: "Movimientos",
      stack: "Stack",
      agenda: "Agenda",
      more: "Más",
      profile: "Perfil",
      voiceAria: "Pagar por voz",
    },
    install: {
      title: "Instalá MidatoPay",
      body: "Tené la app en tu pantalla de inicio para acceder más rápido.",
      cta: "Instalar",
      dismiss: "Ahora no",
      iosTitle: "Instalá MidatoPay",
      iosBody: "En iPhone o iPad, tocá el botón Compartir y elegí \"Agregar a pantalla de inicio\".",
      iosStep1: "Tocá Compartir",
      iosStep2: "Elegí \"Agregar a pantalla de inicio\"",
      iosCta: "Listo",
    },
  },
};

function getPath(obj, path) {
  return path.split(".").reduce((o, k) => (o ? o[k] : undefined), obj);
}

function makeT(lang) {
  return (path, ...args) => {
    const val = getPath(translations[lang], path);
    if (typeof val === "function") return val(...args);
    if (val === undefined) return path;
    return val;
  };
}

const LanguageContext = createContext(null);

export function LanguageProvider({ children }) {
  const [lang, setLangState] = useState(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      return saved === "en" || saved === "es" ? saved : "en";
    } catch {
      return "en";
    }
  });

  const setLang = (l) => {
    setLangState(l);
    try {
      localStorage.setItem(STORAGE_KEY, l);
    } catch {}
  };

  const value = useMemo(
    () => ({ lang, setLang, t: makeT(lang), locale: localeFor(lang) }),
    [lang]
  );

  return <LanguageContext.Provider value={value}>{children}</LanguageContext.Provider>;
}

export function useLanguage() {
  const ctx = useContext(LanguageContext);
  if (!ctx) throw new Error("useLanguage must be used within a LanguageProvider");
  return ctx;
}
